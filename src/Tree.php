<?php
/**
 * The descendant outline: a block for a person's text, and the /tree route.
 *
 * @package Familypedia
 */
...
class Tree {
	const BLOCK_NAME = 'familypedia/tree';
	const CACHE_GROUP = 'familypedia';

	/**
	 * People already expanded in this request, so that a later tree on the same
	 * page links back instead of repeating a subtree.
	 *
	 * @var array
	 */
	...
	public function register_block() {
		wp_register_script(
			'familypedia-tree-block',
			Assets::url( 'tree-block.js' ),
			array( 'wp-blocks', 'wp-element', 'wp-block-editor', 'wp-components', 'wp-server-side-render', 'wp-api-fetch', 'wp-i18n' ),
			Assets::version( 'tree-block.js' ),
			true
		);

		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'familypedia-tree-block', 'familypedia' );
		}
		...
		register_block_type(
			...
			array(
				'api_version'     => 3,
				'title'           => __( 'Family Tree', 'familypedia' ),
				'description'     => __( 'A descendant outline starting from one person.', 'familypedia' ),
				'category'        => 'widgets',
				'icon'            => 'networking',
				'editor_script'   => 'familypedia-tree-block',
				'style'           => 'familypedia-tree-block',
				'attributes'      => array(
					'root'        => array(
						'type'    => 'number',
						'default' => 0,
	...
	public function render( $attributes ) {
		...
		if ( ! $root || ! isset( $people[ $root ] ) ) {
			...
			return '<div ' . get_block_wrapper_attributes( array( 'class' => 'familypedia-tree' ) ) . '><p class="familypedia-tree__notice">'
				. esc_html__( 'Family Tree: pick the person to start this branch from.', 'familypedia' )
				. '</p></div>';
		}

		return '<div ' . get_block_wrapper_attributes( array( 'class' => 'familypedia-tree' ) ) . '>' . self::render_list(
			$root,
			array(
	...
	/**
	 * The outline for one person, for the block and for the /tree route.
	 */
	public static function render_list( $root, $options = array() ) {
		...
		return '<ul class="familypedia-tree__list">' . self::render_person( $root, 1, $options ) . '</ul>';
	}

	/**
	 * Forget which people were already drawn, so a second tree in the same
	 * request starts fresh.
	 */
	public static function reset_expanded() {
		self::$expanded = array();
	}
	...
	private static function render_person( $id, $depth, $options ) {
		...
		if ( ! $options['expand_fully'] && isset( self::$expanded[ $id ] ) ) {
			return '<li class="familypedia-tree__repeat">' . self::person_link( $id, $options )
				. ' <em class="familypedia-tree__seen">' . esc_html__( '(see above)', 'familypedia' ) . '</em></li>';
		}
		...
		if ( $options['max_depth'] && $depth >= $options['max_depth'] ) {
			...
			return '<li>' . $line . ' <a class="familypedia-tree__more" href="' . esc_url( $person['url'] ) . '">'
				. esc_html(
					sprintf(
						// translators: %d is a number of descendants.
						_n( '… %d more', '… %d more', $remaining, 'familypedia' ),
						$remaining
					)
				)
				. '</a></li>';
	...
	private static function person_link( $id, $options ) {
		...
		if ( $options['show_dates'] ) {
			$years = self::year_range( $person );
			if ( $years ) {
				$label .= ' <span class="familypedia-tree__years">(' . esc_html( $years ) . ')</span>';
			}
		}
	...
	private static function build_people() {
		$posts = Person::get_all( array( 'post_parent' => 0 ) );
		...
		foreach ( $posts as $post ) {
			...
			$people[ $id ] = array(
				...
				'url'         => Person::url( $post ),
				'sex'         => (string) Person::field( 'sex', $id ),
				'birth'       => self::year( Person::field( 'birth_date', $id ) ),
				'death'       => self::year( Person::field( 'death_date', $id ) ),
				'father_name' => (string) Person::field( 'father_name', $id ),
				'mother_name' => (string) Person::field( 'mother_name', $id ),
				'parents'     => array(),
				'children'    => array(),
				'partners'    => array(),
			);
			...
			$raw[ $id ] = array(
				'father'    => Person::to_id( Person::field( 'father', $id ) ),
				'mother'    => Person::to_id( Person::field( 'mother', $id ) ),
				'children'  => Person::to_ids( Person::field( 'children', $id ) ),
				'marriages' => Person::field( 'marriages', $id ),
				// The legacy spouse field allows multiple selections.
				'spouse'    => Person::to_ids( Person::field( 'spouse', $id ) ),
				'spouse_nm' => (string) Person::field( 'spouse_name', $id ),
			);
		}
		...
		// Partners, from the marriages list and the legacy spouse fields.
		$partners = array();
		foreach ( $raw as $id => $fields ) {
			$partners[ $id ] = array();
